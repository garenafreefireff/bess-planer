  ...
  return (
    <>
      ...
      <main className="site-container pb-0 pt-5">
        <section>
          <h1 className="text-[42px] font-extrabold leading-tight text-brand-navy">
        ...
        <section className="mt-4 grid grid-cols-[0.72fr_repeat(4,1fr)] gap-4 max-xl:grid-cols-2 max-md:grid-cols-1">
          <div className="flex flex-col justify-center">
            <h2 className="text-2xl font-extrabold text-brand-navy">Bạn cần hỗ trợ gì?</h2>
    ...
    <Card className="bg-white p-5 shadow-none">
      <form className="grid gap-4">
        <div className="grid grid-cols-2 gap-6 max-md:grid-cols-1">
          <InputBox icon={Users} label="Họ tên" placeholder="Nhập họ và tên của bạn" />
          <InputBox icon={BriefcaseBusiness} label="Công ty" placeholder="Nhập tên công ty" />
          <InputBox icon={Mail} label="Email công việc" placeholder="Nhập email công việc" />
          <InputBox icon={Phone} label="Số điện thoại" placeholder="Nhập số điện thoại" />
        </div>
        <label className="grid gap-2 text-sm font-bold text-brand-navy">
          ...
          <span className="relative">
            <select className="h-11 w-full appearance-none rounded-md border border-brand-line bg-white px-4 text-sm font-semibold text-brand-muted outline-none focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/15" defaultValue="">
        ...
        <label className="grid gap-2 text-sm font-bold text-brand-navy">
          ...
          <textarea
            className="min-h-[62px] resize-none rounded-md border border-brand-line bg-white px-4 py-3 text-sm font-semibold outline-none placeholder:text-brand-muted focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/15"
            placeholder="Vui lòng mô tả chi tiết nhu cầu hoặc câu hỏi của bạn"
          />
        </label>
        <button className={buttonVariants({ variant: "green", className: "h-10" })} type="button">
          <Mail size={17} />
          Gửi yêu cầu
        </button>
        <p className="flex items-center justify-center gap-2 text-xs font-semibold text-brand-muted">
          ...
          Chúng tôi cam kết bảo mật thông tin và phản hồi trong 24 giờ làm việc.
        </p>
      </form>
    </Card>
  );
...
function InputBox({ icon: Icon, label, placeholder }: { icon: LucideIcon; label: string; placeholder: string }) {
  return (
    <label className="grid gap-2 text-sm font-bold text-brand-navy">
      ...
      <span className="relative">
        ...
        <input className="h-11 w-full rounded-md border border-brand-line bg-white pl-11 pr-4 text-sm font-semibold outline-none placeholder:text-brand-muted focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/15" placeholder={placeholder} />
      </span>
    </label>
  );
}
...
function ContactInfo() {
  return (
    <Card className="bg-white p-5 shadow-none">
      <h2 className="text-xl font-extrabold text-brand-navy">Thông tin liên hệ</h2>
...
function MapPreview() {
  return (
    <div className="relative min-h-[290px] overflow-hidden rounded-lg border border-brand-line bg-[#eef5ef]">
      ...
      <span className="absolute left-[45%] top-24 rotate-6 text-sm font-extrabold text-brand-blue">Duy Tân</span>
  ...
  return (
    <Card className="grid min-h-[112px] grid-cols-[54px_1fr] items-center gap-4 bg-white p-4 shadow-none">
      ...
      <span>
        ...
        <a className="mt-2 inline-flex items-center gap-2 text-xs font-extrabold text-brand-blue" href="#">
          {action}
          <Send size={13} />
        </a>
      </span>
  ...
  return (
    <Card className="mt-4 bg-white p-5 shadow-none">
      <div className="grid grid-cols-[330px_1fr] gap-8 max-lg:grid-cols-1">
        <div>
          <h2 className="text-xl font-extrabold text-brand-navy">Quy trình làm việc với EnergyInsight</h2>
          ...
          {steps.map(([number, title, text], index) => (
            <div className="contents max-lg:block" key={number}>
              <div className="grid grid-cols-[42px_1fr] gap-4">
                <span className="grid size-9 place-items-center rounded-full bg-brand-blue text-base font-black text-white">{number}</span>
                <span>
                  <strong className="block text-sm text-brand-navy">{title}</strong>
                  <small className="mt-1 block text-xs font-semibold leading-5 text-brand-muted">{text}</small>
                </span>
